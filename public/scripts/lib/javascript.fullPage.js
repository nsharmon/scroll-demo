...
(function( root, window, document, factory, undefined ) {
    ...
    */
    ...
    function setIsScrollable(value, direction){
        switch (direction){
            ...
            case 'all': setAllowScrolling(value);
        }
    }
    ...
    */
    ...
    function moveSlide(direction){
        ...
        if(direction == 'prev' && !isScrollAllowed['left']) return;
        if(direction == 'next' && !isScrollAllowed['right']) return;

        var destiny = getDestination(slides, direction);
        if(!destiny) return;
    ...
    function getDestination(slides, direction) {
        ...
        do {
            ...
            if(destiny.style.display == 'none') {
                currentSlide = destiny;
                destiny = null;
            }
        } while (!destiny);
    ...
    */
    function scrollPage(element, callback, isMovementUp){
        ...
        if (!isScrollAllowed.up && isMovementUp) return;
        if (!isScrollAllowed.down && !isMovementUp) return;
    ...
    */
    ...
    function updateSlideButtons() {
        var current = $(SLIDE_ACTIVE_SEL);
        var section = closest(current, function(e1){
            return hasClass(e1, SECTION);
        });
        ...
        toggle($(SLIDES_ARROW_PREV_SEL, section), !!prev && !options.forceDirection && isScrollAllowed.left);

        //hidding it for the last slide, showing for the rest
        var next = current.nextElementSibling;
        while(next && next.style.display == 'none') next = next.nextElementSibling;
        toggle($(SLIDES_ARROW_NEXT_SEL, section), !!next && isScrollAllowed.right);
    }
